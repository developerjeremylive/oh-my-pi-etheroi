...
- Fixed extension package sub-discovery so `extensions:` entries in project or user `config.yml` now wire sibling `skills/`, `prompts/`, `hooks/`, `rules`, `commands`, tools, and `.mcp.json` without duplicating the path in legacy `settings.json`. ([#1557](https://github.com/can1357/oh-my-pi/issues/1557))
